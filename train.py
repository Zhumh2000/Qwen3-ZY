...
PROMPT = "你是一个中医专家，你需要根据用户的问题，给出带有思考的回答。"
MAX_LENGTH = 2048
...
def dataset_jsonl_transfer(origin_path, new_path):
    ...
    with open(origin_path, "r", encoding="utf-8") as file:
        for line in file:
            ...
            input = data.get("question") or data.get("instruction", "")
...
def select_device_and_dtype():
    """
    优先 CUDA；只要 torch.cuda 可用就交给它自己处理，
    不再因为 major>=12 就人为回退。
    """
    if torch.cuda.is_available():
        try:
            ...
            print(f"WARN: CUDA initialization failed ({e}), fall back to CPU.")
    print("INFO: No usable CUDA device, using CPU for training.")
...
def predict(messages, model, tokenizer, device):
    ...
    generated_ids = model.generate(
        ...
        max_new_tokens=512,  # 不用 MAX_LENGTH，太长
        use_cache=False,  # 关键
